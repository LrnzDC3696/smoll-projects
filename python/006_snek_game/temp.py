...
import curses
from utils import Direction, Object, Status
...
class Model:
    ...
    def process(self, key: Union[str, None]) -> dict[Status, Any]:
        ...
        if self.check_if_die(new_y, new_x):
            return {Status.DEAD: True}

        string = self.move_snek_to(new_y, new_x)
        return {Status.MOVED: (new_y, new_x, string)}
...
class View:
    ...
    def update_str_at(self, y, x, string):
...
class Controller:
    ...
    def handle_key_input(self, key: Union[str, None]) -> None:
        response = self.model.process(key)
        if response.get(Status.DEAD):
            return self.handle_death()
        elif response.get(Status.MOVED):
            return self.view.update_str_at(*response[Status.MOVED])


    def handle_death(self):
        self.view.going_brr = False
